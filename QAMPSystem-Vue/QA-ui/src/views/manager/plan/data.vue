...
>
  ...
  <el-form ref="planRef" :model="form" :rules="rules" label-width="100px" class="compact-form">
    <el-row :gutter="20">
      <el-col :span="12">
        <el-form-item label="计划标识：" prop="planId">
    ...
    status: [
      ...
      { 
        validator: (rule, value, callback) => {
          if (form.value.submitterId && !value) {
            callback(new Error("请选择提测时间"));
          } else {
            callback();
          }
        },
...
queryParams.value.reqId = route.params.reqId;
/** 查询测试计划列表 */
function getList() {
  loading.value = true;
...
// 新增初始化加载逻辑
onMounted(() => {
  if (route.params.reqId) {
    getList() // 自动触发带需求ID的查询
  }
})
...
function getRequirementList() {
  ...
  listRequirement(loadAllParams).then(response => {
    ...
    loading.value = false;
    if (route.params.reqId) {
      const targetReq = requirementOptions.value.find(
        item => item.id == route.params.reqId
      );
      if (targetReq) {
        queryParams.value.reqId = targetReq.id;
      }
    }
  });
...
watch(
  ...
  (newVal) => {
    if (!newVal) {
      form.value.submitTime = null; // 清空提测时间
    }
  }
);
